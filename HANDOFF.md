...
- **口径**：换手用 000300 成交额代理；IV 缺历史时回退 20 日已实现波动；有效历史 <60 交易日的指标强制记 0 并标记 `cold_start`（避免期权日表冷启动 PCR 打出 ±1）
- **双版本信号**：`v1` 标准版（五维 10 指标，阈值 ±0.33，合理区间 0.25~0.35）；`v2` 震荡优化版（去量/价 6 指标，阈值 ±0.20，合理区间 0.15~0.25）。十年 H00300 阈值体检见 `engine/hs300_score_sensitivity.py`。门户可切换；payload `signals.v1/v2` + 序列字段 `s`/`s2`
